	...
	<?php //Outputs non-minfied functions file for local testing
	/*
		if (strpos($_SERVER['HTTP_HOST'],'local') !== false) {
			wp_enqueue_script('custom-js',get_bloginfo('template_directory').'/js/functions.js',array('jquery','browser'),date('Y-m-d-h-i-s'));
		} else {
			$js = dirname(__FILE__).'/js/functions.js';
			if (file_exists($js)) {$js_date = date ("Y.m.d", filemtime($js));}
			wp_enqueue_script('custom-js',get_bloginfo('template_directory').'/js/functions.min.js',array('jquery','browser'),$js_date);
		}	
		*/
		wp_enqueue_script('custom-js',get_bloginfo('template_directory').'/js/functions.js',array('jquery','browser'),date('Y-m-d-h-i-s'));
	?>
